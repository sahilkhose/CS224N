...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) !=0:
        ...
        for transition, parse_sen in zip(transitions, minibatch):
            parse_sen.parse_step(transition)
            if len(parse_sen.buffer) == 0 and len(parse_sen.stack) == 1:
                unfinished_parses.remove(parse_sen)

    dependencies = [partial_parse.dependencies for partial_parse in partial_parses]
    # dependencies.extend([partial_parse.dependencies for partial_parse in partial_parses])
    ### END YOUR CODE
