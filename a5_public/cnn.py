...
class CNN(nn.Module):
    ...
    def __init__(self, kernel_size=5, padding=1):
    	...
    	self.conv = nn.Conv1d(in_channels=, out_channels=, kernel_size=self.kernel_size, padding=self.padding)
    ...
    def forward(self, X_reshaped: torch.Tensor):
    	...
    	return the output after passing the input through a Conv1d network
   		...
   		@param X_reshaped (Tensor): tensor of shape (b, )
   								b = batch size

   		@returns X_conv_out (Tensor): tensor of shape (b, )
